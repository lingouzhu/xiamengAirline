...
public class LocalSearch2Backup {

	private static final Logger logger = Logger.getLogger(LocalSearch2Backup.class);
	...
	private BigDecimal adjust2(XiaMengAirlineSolutionBackup newSolution, XiaMengAirlineSolutionBackup oldSolution) throws CloneNotSupportedException, SolutionNotValidBackup {
		...
		try {
			XiaMengAirlineSolutionBackup aBetterSolution = newSolution.getBestSolution();
			XiaMengAirlineSolutionBackup aBetterOutput = aBetterSolution.reConstruct2();
			aBetterOutput.refreshCost(false);
			return aBetterOutput.getCost();
		} catch (AircraftNotAdjustableBackup ex) {
	...
	private BigDecimal adjust(XiaMengAirlineSolutionBackup newSolution, XiaMengAirlineSolutionBackup oldSolution) throws CloneNotSupportedException, SolutionNotValidBackup {
		...
		if (!newSolution.validflightNumers(oldSolution)) {
			throw new SolutionNotValidBackup(newSolution, "exchange");
		}
		

		XiaMengAirlineSolutionBackup backup = newSolution.clone();
		if (newSolution.adjust()) {
			if (!newSolution.validAlternativeflightNumers(oldSolution) 
				...
				throw new SolutionNotValidBackup(newSolution, "adjust");
			}
