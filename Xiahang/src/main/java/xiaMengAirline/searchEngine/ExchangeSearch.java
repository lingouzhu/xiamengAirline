...
public class ExchangeSearch {
	...
	private SelfSearch adjustmentEngine = null;
						...
						for (int uu = 0; uu <= m - 1; uu++) {
							...
							if (circuitFlightsAir1.containsKey(flightAir1)) {
								for (Flight destFlight : circuitFlightsAir1.get(flightAir1)) {
									...
									// ...");
									// List<Flight> updateList1 =
									// newAircraft1.getFlightChain();
									// for (Flight aF : updateList1) {
									// logger.debug("Air " +
									// newAircraft1.getId() + " flight " +
									// aF.getFlightId());
									// }
									// List<Flight> updateList2 =
									// cancelledAir.getFlightChain();
									// for (Flight aF : updateList2) {
									// logger.debug("Air cancelled " +
									// cancelledAir.getId() + " flight "
									// + aF.getFlightId());
									// }
									...
									if (adjustmentEngine.adjust(newAircraft1)) {
										if (newAircraft1.getCost() < air1.getCost()) {
											logger.debug(
													"Better Solution exists! Method 1 : " + newAircraft1.getCost());
											...
											if (BusinessDomain.validateFlights(air1, air1Cancelled, newAircraft1,
												...
												aBetterSolution.refreshCost(new BigDecimal(newAircraft1.getCost() - air1.getCost()));
						...
						for (int xx = 0; xx <= n - 1; xx++) {
							...
							if (circuitFlightsAir2.containsKey(flightAir2)) {
								for (Flight destFlight : circuitFlightsAir2.get(flightAir2)) {
									...
									// ...");
									// List<Flight> updateList1 =
									// newAircraft2.getFlightChain();
									// for (Flight aF : updateList1) {
									// logger.debug("Air " +
									// newAircraft2.getId() + " Flights: " +
									// aF.getFlightId());
									// }
									// List<Flight> updateList2 =
									// cancelledAir.getFlightChain();
									...
									if (adjustmentEngine.adjust(newAircraft2)) {
										if (newAircraft2.getCost() < air2.getCost()) {
											logger.debug(
													"Better Solution exists! Method 2 : " + newAircraft2.getCost());
											if (aStragety.isAbortWhenImproved())
												isImproved = true;
											if (BusinessDomain.validateFlights(air2, air2Cancelled, newAircraft2,
												...
												aBetterSolution.replaceOrAddNewAircraft(cancelledAir);
												aBetterSolution.refreshCost(new BigDecimal(newAircraft2.getCost() - air2.getCost()));
												neighboursResult.addSolution(aBetterSolution);
											} else {
					...
					for (int u = 0; u <= m - 1; u++) {
						...
						if (matchedFlights.containsKey(air1.getFlight(u)) && !isImproved) {
							...
							for (MatchedFlight aMatched : matchedList) {
								...
								// for (Flight aF : updateList1) {
								// logger.debug("Air " +
								// newAircraft1.getId() + "isCancel " +
								// newAircraft1.isCancel()
								...
								// for (Flight aF : updateList2) {
								// logger.debug("Air " +
								// newAircraft2.getId() + "isCancel " +
								// newAircraft2.isCancel()
								// + " flight " + aF.getFlightId());
								// }
								...
								if (adjustResult) {
									...
									if (newCost < oldCost) {
										logger.debug("Better Solution exists! Method 3 : " + newCost);
										if (aStragety.isAbortWhenImproved())
											isImproved = true;
							...
							if (circuitFlightsAir1.containsKey(flightAir1)) {
								for (int x = 0; x <= n - 1; x++) {
									for (Flight destFlight : circuitFlightsAir1.get(flightAir1)) {
										...
										// logger.debug("Method 4 After
										// exchange ...");
										// List<Flight> updateList1 =
										// newAircraft1.getFlightChain();
										// for (Flight aF : updateList1) {
										// logger.debug(
										// "Air " + newAircraft1.getId() + "
										// Flights: " + aF.getFlightId());
										// }
										// List<Flight> updateList2 =
										// newAircraft2.getFlightChain();
										// for (Flight aF : updateList2) {
										// logger.debug(
										// "Air " + newAircraft2.getId() + "
										// Flights: " + aF.getFlightId());
										// }
										// logger.debug("Method 4 Complete
										// exchange ...");
										...
										if (adjustResult) {
											...
											if (newCost < oldCost) {
												logger.debug("Better Solution exists! Method 4 : " + newCost);
												if (aStragety.isAbortWhenImproved())
													isImproved = true;
							...
							for (int x = 0; x <= n - 1; x++) {
								...
								if (circuitFlightsAir2.containsKey(flightAir2)) {
									for (Flight destFlight : circuitFlightsAir2.get(flightAir2)) {
										...
										// logger.debug("Method 5 After
										// exchange ...");
										// List<Flight> updateList1 =
										// newAircraft1.getFlightChain();
										// for (Flight aF : updateList1) {
										// logger.debug("Air " +
										// newAircraft1.getId() + " Flights:
										// "
										// + aF.getFlightId());
										// }
										// List<Flight> updateList2 =
										...
										// + aF.getFlightId());
										// }
										// logger.debug("Method 5 Complete
										// exchange ...");
										...
										if (adjustResult) {
											...
											if (newCost < oldCost) {
												logger.debug("Better Solution exists! Method 5 : " + newCost);
												if (aStragety.isAbortWhenImproved())
													isImproved = true;
												if (BusinessDomain.validateFlights(air1, air2, newAircraft1,
														newAircraft2)) {
...
//									logger.debug("Method 6 Complete exchange ...");
									...
									if (adjustResult) {
										...
										if (newCost < oldCost) {
											logger.debug("Better Solution exists! Method 6 : " + newCost);
											if (aStragety.isAbortWhenImproved())
												isImproved = true;
											if (BusinessDomain.validateFlights(air1, air2, newAircraft1,
													newAircraft2)) {
	...
	public SelfSearch getAdjustmentEngine() {
		return adjustmentEngine;
	}

	public void setAdjustmentEngine(SelfSearch adjustmentEngine) {
