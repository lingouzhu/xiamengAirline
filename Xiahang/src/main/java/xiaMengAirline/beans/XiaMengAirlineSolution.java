...
//								System.out.println(hourDiff);
						...
						if (InitData.jointFlightMap.get(newFlight.getFlightId()) != null) {
							if (!newFlight.getDesintationAirport().getId().equals((newFlight.getPlannedFlight().getDesintationAirport().getId()))) {
								...
								isStretch = true;
								if (newFlight.getFlightId() == 1338){
									System.out.println(CSVUtils.flight2Output(newFlight, aAir.getId(), "0", "1", "0"));
									System.out.println(CSVUtils.flight2Output(nextFlight, aAir.getId(), "0", "1", "0"));
								}
								if (refreshOut) {
